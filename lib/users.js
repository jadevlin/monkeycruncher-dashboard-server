...
exports.register = function (failurePage, successPage) {
    return function (request, response, next) {
        var body = request.body;
        if (body.username && body.password && body.email) {
            create(body.username, body.password, body.email, function (err) {
                if (err) {
...
var create = function (username, password, email, callback) {
    ...
    db.connect(function (err, client) {
        ...
        client.query(
            "INSERT INTO users (username, password_hash, email, active, admin) VALUES( $1, $2, $3, false, false )",
