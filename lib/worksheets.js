...
exports.addWorksheetToDB = function (userID, worksheetName, uuid, parent, callback) {
    db.connect(function (err, client) {
        if (err) return callback(err);
        client.query(
            "INSERT INTO worksheets (name, owner, parent, last_edited, document_ref) VALUES ($1, $2, $3, NOW(), $4)",
...
exports.loadWorksheetFromDocumentRef = function (documentRef, callback) {
    db.connect(function (err, client) {
        ...
        client.query(
            "SELECT * FROM worksheets WHERE document_ref = $1",
            [documentRef],
            function (err, result) {
                if (err) return callback(err);
                if (result.rowCount !== 1) return callback(new Error("Worksheet not found."));
...
exports.claim = function (userID, uuid, callback) {
    db.connect(function (err, client) {
        ...
        client.query(
            "UPDATE worksheets SET owner = $1 WHERE document_ref = $2 AND owner IS NULL",
            [userID, uuid],
            function (err, result) {
                if (err) return callback(err);
