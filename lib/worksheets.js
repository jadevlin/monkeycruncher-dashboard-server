...
exports.remove = function (editServerURL, editServerSecret, worksheetID, documentUUID, callback) {
    console.log("Removing worksheet ID " + worksheetID);
    db.connect(function (err, client) {
        if (err) return callback(err);
        client.query(
...
exports.loadAllWorksheets = function (userID, callback) {
    db.connect(function (err, client) {
        ...
        client.query(
            "SELECT * FROM worksheets WHERE owner = $1 AND deleted = FALSE",
            [userID],
            function (err, result) {
                if (err) return callback(err);
