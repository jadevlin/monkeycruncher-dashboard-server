                ...
                if (user) {
                    ...
                    if (bcrypt.compareSync(credentials.password, user.passwordHash)) {
                        ...
                        if (user.active) {
                            ...
                            session.message = "This account is not activated.";
                            response.redirect(failurePage);
                        }
...
// then the request is passed to the next middleware in the stack. If the session is
// not authenticated then an error message (JSON) is returned.
auth.requireAuthenticated = function (request, response, next) {
